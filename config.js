const Guild = require("./codebase/models/Guild");
const mongoose = require("mongoose");
const Member = require("./codebase/models/Member");

module.exports = new (require("./types/Config"))({
  // E tabi, bot tokeni buraya.
  clientToken: "ODI0MjEwMTMyMzUwMDA5MzY2.YFsDgA.AcmhnScdpq3rgml3KUob3EWAReg",
  ...
  other: {
    ...
    banHammer: "",
    jailHammer: "",
    timeoutHammer: "", // uzaklaştırma

    discriminatorTag: "2300",
    tag: "xate",

    projectName: "Xate #2300",

    blockedUsernameContents: [""], // işte yasaklı taglar falan buraya geliyor
  },
  ...
  async onBeforeLoad(client) {
    console.log("[CONFIG] Yüklemeye başlamadan önce çalıştı.");
    await mongoose.connect("mongodb://localhost/xate", {
      auth: {
        username: "armagan",
        password: "dWj4mAvFLGTbwe2FRQgz6wNY"
      },
      authSource: "admin"
    })
  },
  ...
  async onReady(client) {
    ...
    client.user.setActivity(`Armagan#4869 ❤ ${Underline.other.projectName}`, { type: "WATCHING" });
    setInterval(() => {
      client.user.setActivity(`Armagan#4869 ❤ ${Underline.other.projectName}`, { type: "WATCHING" });
    }, 60000);
  },
  ...
  async onInteraction(uInteraction, interaction, other) {
    let dbGuild = await Guild.findOneAndUpdate({ guildId: interaction.guildId }, {}, { upsert: true, new: true }).exec();
    other.dbGuild = dbGuild;
    let dbMember = await Member.findOneAndUpdate({ guildId: interaction.guildId, userId: interaction.user.id }, {}, { upsert: true, new: true }).exec();
    other.dbMember = dbMember;
    return true;
  },
